...
export class ArtifactOperation {
  ...
  async registerDownload(download: DownloadLike) {
    if (this.sealed) return this.result; this.events++; if (this.events > 1) { this.generation++; this.sealed = true; this.store.discardArtifact(this.artifactId); this.release(); this.result = { outcome: "capture-failed", failure: "multiple-artifacts" }; return this.result; }
    ...
    if (failure) { this.release(); this.result = { outcome: "capture-failed", failure: "download-capture-failed" }; return this.result; }
    let path: string | null;
    try { path = await download.path(); } catch { path = null; }
    if (this.sealed || generation !== this.generation) return this.result;
    if (!path) { this.release(); this.result = { outcome: "capture-failed", failure: "download-capture-failed" }; return this.result; }
    ...
    if (captured.status === "available") this.result = { outcome: "available", artifact: captured }; else if ("failure" in captured) { this.release(); this.result = { outcome: "capture-failed", failure: captured.failure }; }
    return this.result;
  }
  seal(): OperationResult { if (this.sealed) return this.result ?? { outcome: "capture-failed", failure: "artifact-runtime-invalidated" }; this.sealed = true; this.generation++; const essence = ((this.contentType ?? "").trim().toLowerCase().split(";", 1)[0] ?? "").trim(); if (this.events === 0 && this.status === 200 && essence === "application/pdf") this.result = { outcome: "inline-pdf-unsupported", failure: "inline-pdf-unsupported" }; if (!this.result || this.result.outcome !== "available") this.release(); return this.result ?? { outcome: "none" }; }
  invalidate() { this.sealed = true; this.generation++; this.store.discardArtifact(this.artifactId); this.release(); this.result = { outcome: "capture-failed", failure: "artifact-runtime-invalidated" }; return this.result; }
}
