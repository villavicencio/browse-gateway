...
export class ArtifactOperation {
  ...
  async registerDownload(download: DownloadLike) {
    if (this.sealed) return; this.events++; if (this.events > 1) { this.store.discard(this.id); this.result = { status: "multiple-artifacts" }; return; }
    ...
    if (this.sealed || generation !== this.generation) { this.store.discard(this.id); return; }
  ...
  seal(): OperationResult { if (this.sealed) return this.result ?? { status: "capture-failed" }; this.sealed = true; this.generation++; const essence = this.contentType.trim().toLowerCase().split(";", 1)[0] ?? ""; if (this.events === 0 && essence.trim() === "application/pdf") this.result = { status: "unsupported-inline" }; else if (this.events > 1) { this.store.discard(this.id); this.result = { status: "multiple-artifacts" }; } return this.result ?? { status: "capture-failed" }; }
  invalidate() { this.sealed = true; this.generation++; this.store.discard(this.id); this.result = { status: "capture-failed" }; return this.result; }
}

export class ArtifactRuntime {
  readonly store: ArtifactStore;
  constructor(options: ArtifactStoreOptions) { this.store = new ArtifactStore(options); }
  createOperation(consumerId: string, sourceHost: string, id = randomBytes(16).toString("base64url")) { if (!/^(?=.{1,253}$)([A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(\.[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*$/.test(sourceHost)) throw new Error("invalid source host"); return new ArtifactOperation(this.store, consumerId, id); }
