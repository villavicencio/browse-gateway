 ...
 * + the clearance poll of each proxied attempt) with a pre-attempt bail once too little budget remains — the
 * same shape as retrieve. KTD-5 still holds: a PINNED (post-first-navigate) session never re-rolls its exit
 * mid-flow; the budget just bounds that single navigate. The env-overridable per-stage timeouts below apply
 * to both verbs.
 */
