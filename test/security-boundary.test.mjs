...
test("redactSecrets: scrubs the JSON AND util.inspect escaped forms of a secret (incl. control chars)", () => {
  // A secret with quote/backslash/control chars is transformed differently by each serializer; both
  // escaped forms must be matched (JSON and util.inspect diverge — e.g. quote escaping, \\u00 vs \\x00).
  const secret = 'p@ss"w\\rd-long';
  const store = new SecretStore(() => ({ BGW_PROXY_PASSWORD: secret }));
  const jsonForm = JSON.stringify(secret).slice(1, -1);
  const inspectForm = inspect(secret, { maxStringLength: Infinity }).slice(1, -1);
  assert.notEqual(jsonForm, inspectForm, "the two serializers really do escape this secret differently");
  const out = redactSecrets(`raw=${secret} json=${jsonForm} inspect=${inspectForm}`, store);
  assert.ok(!out.includes(secret), "raw form scrubbed");
  assert.ok(!out.includes(jsonForm), "JSON-escaped form scrubbed");
  assert.ok(!out.includes(inspectForm), "util.inspect-escaped form scrubbed");
});
