...
2. ~~File the VIL-112 breakdown~~ — **DONE.** VIL-112 narrowed to deployment identity and reparented under the new release-engineering epic **VIL-125** (filed BLOCKED on the three decisions in item 1, and carrying the four verified-broken repairs so they are not rediscovered). **VIL-126** takes the budget half plus the backpressure signal and the `poolSizingError` message fix; **VIL-127** takes structured `failureClass` and blocks VIL-125. VIL-126 and VIL-127 sit under VIL-90.
